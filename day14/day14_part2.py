...
# --- Part Two ---
# You realize you misread the scan. There isn't an endless void at the bottom of the scan - there's floor, 
...
class Cave:
    ...
    def store_infinte_floor(self):
        self.infinite_floor_y = self.max_y + 2
    ...
    def can_fall(self, x, y):
        # so we can't fall if something is already there...
        can_fall = (x, y) not in self.places
        # we also cannot fall beyond the infinite floor..
        if y == self.infinite_floor_y:
            can_fall = False
        return can_fall
    ...
    def pour_one_sand(self):
        # nothing falls forever now.. just drop one more bag of sand..
        x, y = 500, 0
...
def load_cave_from_file(filename: str):
    ...
    # lock in the floor height now
    cave.store_infinte_floor()
