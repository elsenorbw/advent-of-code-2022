...
def walk_the_elves(elves, iteration: int):
    ...
    planned_moves = {}
    currently_populated_locations = set(elves)
    ...
    for this_elf in elves:
        ...
        populated_locations = [
            (x, y) in currently_populated_locations for x, y in view_locations
        ]
    ...
    start_positions = set(planned_moves.keys())
    end_positions = set(planned_moves.values())
    difference = start_positions.symmetric_difference(end_positions)
    any_moves = len(difference) > 0
    return result, any_moves
...
def part2(filename: str):
    ...
    someone_moved = True
    while someone_moved:
        elves, someone_moved = walk_the_elves(elves, this_iteration)
        print(f"Elves after iteration {this_iteration} ({someone_moved})")
        # print_elf_field(elves)
        this_iteration += 1
    ...
    print(f"after {result} iterations, the elves in {filename} stopped moving around")
    return result
