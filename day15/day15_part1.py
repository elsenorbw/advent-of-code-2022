...
class Sensor:
    ...
    def exclude_range(self, target_y: int):
        ...
        if remaining_distance > 0:
            result = (self.x - remaining_distance, self.x + remaining_distance)
        return result
...
def part1(filename: str, target_y: int):
    ...
    # get the location counts for each range..
    range_counts = [a[1] - a[0] for a in merged_ranges]
    print(range_counts)

    # calculate the total..
    total_range_count = sum(range_counts)

    result = total_range_count
    print(f"puzzle result for {filename} at line {target_y} is {result}")
    return result
...
if __name__ == "__main__":
    ...
    sample_y_value = 10
    sample_expected_result = 26
    puzzle_y_value = 2000000

    sample_actual_result = part1(sample_filename, sample_y_value)
    assert sample_actual_result == sample_expected_result

    puzzle_result = part1(puzzle_filename, puzzle_y_value)
