...
# Since you're so good at dodging blizzards, the Elves humbly request that you go back for his snacks. From the same initial conditions, how quickly can you make it
# from the start to the goal, then back to the start, then back to the goal?
...
class Maze:
    ...
    def solve(self, forwards: bool, start_minute: int):
        # solve the maze in the shortest time possible..
        best_so_far = None

        if forwards:
            the_start = self.start_location
            the_end = self.end_location
        else:
            the_start = self.end_location
            the_end = self.start_location

        positions_to_evaluate = OrderedExplorer()
        positions_to_evaluate.add_item(
            the_start,
            start_minute,
            manhattan_distance(the_start, the_end),
        )
        duplicate_prevention = set((the_start, start_minute))
        position_idx = 0
        ...
        while positions_to_evaluate.has_remaining_items():
            ...
            for this_one in additional_locations_to_evaluate:
                ...
                this_manhattan = manhattan_distance(this_location, the_end)
                positions_to_evaluate.add_item(
...
def part2(filename: str):
    ...
    a = maze.solve(forwards=True, start_minute=0)
    b = maze.solve(forwards=False, start_minute=a)
    c = maze.solve(forwards=True, start_minute=b)
    result = c
    ...
    print(
        f"The fastest path across the maze, back again and then across once more in {filename} is {walk1}, {walk2}, {walk3} for a total of {result} minutes"
    )
    return result
