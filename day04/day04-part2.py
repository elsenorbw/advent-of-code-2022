...
def has_any_overlap(the_pair):
    # should be easy enough, if the start or end of b is within the range for a then it's some sort of overlap
    a, b = the_pair

    # important to make sure that the potentially completely covering range is in the "a" slot..
    # if not, then we're potentially going to miss the case where both points of b are wider than the a range, which is a problem
    if b[0] < a[0]:
        a, b = b, a
...
def part2(filename: str):
    ...
    overlaps = [has_any_overlap(this_pair) for this_pair in the_pairs]

    # add em up..
    total_overlaps = sum(overlaps)

    # and that's the answer..
    print(f"part2 result for {filename} is {total_overlaps}")
