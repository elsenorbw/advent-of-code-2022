...
class TheMachine:
    ...
    def cycle_execution(self):
        # This happens each time we cycle, we should draw a pixel one way or the other..
        this_pixel = '.'
        sprite_locations = range(self.x - 1, self.x + 2, 1)
        screen_x = self.screen_position % 40
        if screen_x in sprite_locations:
            this_pixel = '#'

        self.screen.append(this_pixel)
    ...
    def print_screen(self):
        pixel_count = len(self.screen)
        total_rows = pixel_count // 40
        for this_row in range(total_rows):
            row_data = self.screen[this_row * 40: this_row * 40 + 40]
            the_row = "".join(row_data)
            print(the_row)
        print("")

    def get_screen(self):
        result_data = self.screen[:40 * 6]
...
def part2(filename: str):
    ...
    result = machine.get_screen()

    print(f"Answer for {filename} is {result}")
